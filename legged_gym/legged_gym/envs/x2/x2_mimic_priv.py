...
sys.path.append("/home/descfly/Lingyun/exbody2")
sys.path.append("/home/descfly/Lingyun/exbody2/humanoidverse/isaac_utils")
import cv2

# Use MotionLibRobotWTS instead of MotionLib
from humanoidverse.utils.motion_lib.motion_lib_robot_WTS import MotionLibRobotWTS
from easydict import EasyDict
import torch_utils

class X2MimicPriv(LeggedRobot):
    def __init__(self, cfg: LeggedRobotCfg, sim_params, physics_engine, sim_device, headless):
        ...
        self.cfg.x2_params.vel_factor = 1.0
        ...
        if cfg.motion.num_envs_as_motions:
            self.cfg.env.num_envs = self._motion_lib._num_unique_motions
    ...
    def init_motion_buffers(self, cfg):
        ...
        # MotionLibRobotWTS doesn't have get_motion_difficulty, use zeros as placeholder
        self._motion_difficulty = torch.zeros(self.num_envs, device=self.device)

        # MotionLibRobotWTS doesn't have ground offset methods, use zeros as placeholder
        self._motion_left_ground_offsets = torch.zeros(self.num_envs, device=self.device)
        self._motion_right_ground_offsets = torch.zeros(self.num_envs, device=self.device)
    ...
    def _load_motion(self, motion_lib_cfg, no_keybody=False):
        # Initialize MotionLibRobotWTS
        # Use cfg.env.num_envs if available, otherwise use a default
        num_envs = getattr(self.cfg.env, 'num_envs', 4096) if hasattr(self, 'cfg') else 4096
        self._motion_lib = MotionLibRobotWTS(motion_lib_cfg=motion_lib_cfg,
                                              num_envs=num_envs,
                                              device=self.device)
        # Load motions
    ...
    def update_motion_ids(self, env_ids):
        ...
        # MotionLibRobotWTS doesn't have get_motion_difficulty, keep existing values
        # self._motion_difficulty[env_ids] = self._motion_lib.get_motion_difficulty(self._motion_ids[env_ids])
        # MotionLibRobotWTS doesn't have ground offset methods, keep existing values
    ...
    def reset_idx(self, env_ids, init=False):
        ...
        # MotionLibRobotWTS returns a dict instead of tuple
        ...
        dof_pos_motion = motion_state['dof_pos']
        ...
        # # clip initial height
        # self.cfg.x2_params.vel_factor = random.uniform(0.0, 1.5)
        # if self.cfg.x2_params.vel_factor < 0.2:
        #     self.cfg.x2_params.vel_factor = 0.0
        
        root_pos[:, :2] = self.cfg.x2_params.height_factor * root_pos[:, :2]
        root_pos[:, 2] = torch.clamp(root_pos[:, 2], self.cfg.x2_params.min_init_height, self.cfg.x2_params.max_init_height)
        # clip initial velocity
        root_vel = self.cfg.x2_params.height_factor * root_vel * self.cfg.x2_params.vel_factor
        root_vel[:, 0] = torch.clamp(root_vel[:, 0], -self.cfg.x2_params.max_vel, self.cfg.x2_params.max_vel)
        root_vel[:, 1] = torch.clamp(root_vel[:, 1], -self.cfg.x2_params.max_vel, self.cfg.x2_params.max_vel)
        root_vel[:, 2] = torch.clamp(root_vel[:, 2], -self.cfg.x2_params.max_vel, self.cfg.x2_params.max_vel)
    ...
    def update_demo_obs(self):
        ...
        # MotionLibRobotWTS returns a dict, and doesn't have get_lbp parameter
        motion_state = self._motion_lib.get_motion_state(
            self._motion_ids.repeat_interleave(self._motion_num_future_steps), 
    ...
    def _motion_sync(self):
        ...
        # MotionLibRobotWTS returns a dict
        motion_state = self._motion_lib.get_motion_state(motion_ids, self._motion_times)
        root_pos = motion_state['root_pos']
        root_rot = motion_state['root_rot']
        dof_pos = motion_state['dof_pos']
        root_vel = motion_state['root_vel']
        root_ang_vel = motion_state['root_ang_vel']
        dof_vel = motion_state['dof_vel']
        key_pos = motion_state['rg_pos'][:, self._key_body_ids.cpu().numpy()]
            ...
            for evt in self.gym.query_viewer_action_events(self.viewer):
                ...
                if not self.free_cam:
                    ...
                    if evt.action == "next_motion" and evt.value > 0:
                        self._motion_ids[self.lookat_id] = (self._motion_ids[self.lookat_id] + 1) % self._motion_lib._num_unique_motions
