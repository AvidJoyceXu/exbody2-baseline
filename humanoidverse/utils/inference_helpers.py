...
def export_he(path, exported_policy_name, wm):
        os.makedirs(path, exist_ok=True)
        path = os.path.join(path, exported_policy_name)

        history_encoder = copy.deepcopy(wm.history_encoder).to('cpu')

        class HEWrapper(nn.Module):
            def __init__(self, history_encoder):
                """
                model: The original PyTorch model.
                input_keys: List of input names as keys for the input dictionary.
                """
                super(HEWrapper, self).__init__()
                self.history_encoder = history_encoder

            def forward(self, history_with_curr, motion_label):
                """
                Dynamically creates a dictionary from the input keys and args.
                """
                # he_obs: 1,60,52
                # motion_label: 1,10
                return self.history_encoder(history_with_curr,motion_label) # 1,32   

        wrapper = HEWrapper(history_encoder)
        # example_input_list = example_obs_dict["actor_obs"]
        # actor_obs: ['actions', 'base_ang_vel', 'dof_pos', 'history_actor', 'motion_label', 'projected_gravity', 'ref_motion_phase'], sorted, (1,585)
        example_history_with_curr = torch.zeros((1,60,52),device='cpu',dtype=torch.float)
        example_motion_label = torch.zeros((1,10),device='cpu',dtype=torch.float)
        torch.onnx.export(
            wrapper,
            (example_history_with_curr,example_motion_label),  # Pass x1 and x2 as separate inputs
            path,
            verbose=True,
            input_names=["history_with_curr",'motion_label'],  # Specify the input names
            output_names=["motion_phase_embedding"],       # Name the output
            # opset_version=17          # Specify the opset version, if needed
            export_params=True,
            opset_version=13,
            do_constant_folding=True,
