...
def dict_to_easydict(d):
    """Recursively convert a dictionary to EasyDict."""
...
def load_config_from_yaml(yaml_path):
    """Load configuration from a YAML file and convert to EasyDict."""
    with open(yaml_path, 'r') as f:
        config_dict = yaml.load(f, Loader=yaml.SafeLoader)
    return dict_to_easydict(config_dict)
...
def main():
    ...
    # Default to the config file in the same directory as this script
    script_dir = Path(__file__).parent
    config_path = script_dir / "x2_robot_config.yaml"
    
    if not config_path.exists():
        # Fallback: try absolute path
        config_path = Path("/home/descfly/Lingyun/exbody2/humanoidverse/utils/motion_lib/x2_robot_config.yaml")
        if not config_path.exists():
    ...
    # # Ensure extend_config is a list (can be None or empty list)
    # if config.extend_config is None:
    #     config.extend_config = []
    
    humanoid_fk = Humanoid_Batch(config, device)
    humanoid_fk.mesh_fk()
